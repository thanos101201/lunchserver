...
function GameSession() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://lunchtime-bice.vercel.app/session/active/${user}`).then(async (response) => {
            if(response.data.data !== undefined && response.data.data.length > 0){
                ...
                axios.get(`https://lunchtime-bice.vercel.app/session/question/${response.data.data[0].restaurantName}`).then((response2) => {
                    setOption1(response2.data.data[0]);
                    setOption2(response2.data.data[1]);
                    setOption3(response2.data.data[2]);
                    axios.get(`https://lunchtime-bice.vercel.app/user/${user}`).then((response3) => {
  ...
  const updatePoints = async() => {
    ...
    return axios.put('https://lunchtime-bice.vercel.app/session', {
